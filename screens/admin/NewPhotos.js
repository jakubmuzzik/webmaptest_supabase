...
const NewLadies = ({ newPhotos, toastRef, fetchNewPhotos, setNewPhotos, index }) => {
    ...
    useEffect(() => {
        if (newPhotos === null) {
            ...
            let groupedLadiesPhotos = Object.groupBy(newPhotos.filter(newPhoto => newPhoto.lady_id), ({ lady_id }) => lady_id)
            Object.keys(groupedLadiesPhotos).forEach(ladyId => {
                groupedLadiesPhotos[ladyId] = {
                    data: groupedLadiesPhotos[ladyId],
                    ref: React.createRef()
                }
            })

            let groupedEstPhotos = Object.groupBy(newPhotos.filter(newPhoto => newPhoto.establishment_id), ({ establishment_id }) => establishment_id)
            Object.keys(groupedEstPhotos).forEach(estId => {
                groupedEstPhotos[estId] = {
                    data: groupedEstPhotos[estId],
                    ref: React.createRef()
                }
            })
            setPhotos({
                        ...
                        <View style={[styles.section, { marginBottom: SPACING.small }]}>
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginLeft: SPACING.small }}>
                                {photos.ladies[ladyId].data.map(photo => (
                                    <View key={photo.id} style={{ width: cardWidth, marginBottom: SPACING.medium, marginRight: SPACING.small }}>
                                        <NewPhoto photo={photo} width={cardWidth} actions={photoActions} offsetX={windowWidth * index} />
                                    </View>
                                ))}
                            </View>
                        </View>
...
export default connect(mapStateToProps, { fetchNewPhotos, setNewPhotos })(memo(NewLadies))
